...
const STORAGE_KEYS = {
    name: 'animalquest_player_name',
    tag: 'animalquest_player_tag',
    icon: 'animalquest_player_icon',
    player: 'animalquest_player_identity_v2',
    session: 'animalquest_active_session_v2',
    myGames: 'animalquest_my_games',
    darkMode: 'animalquest_dark_mode',
    diagnostics: 'animalquest_diagnostics_visible',
    reactHintSeen: 'animalquest_react_hint_seen_v1'
};

const SESSION_KEYS = { joinReloadCode: 'animalquest_join_reload_code_v1' };
const LEGACY_STORAGE_KEYS = { playerProfile: 'animalquest_player_profile', activeSession: 'animalquest_active_session' };
...
async function auditMyGames() {
    ...
    try {
        const snap = await database.ref('animalquest/games').once('value');
        const raw = snap.val() || {};
...
function checkAppVersion() {
    ...
    if (database) {
        const vRef = database.ref('animalquest/config/latestVersion');
    ...
    const renderModal = (changelog, resolvedLatest) => {
        ...
        overlay.innerHTML = `
            <div class="update-modal-card">
                ...
                <div class="update-modal-title">What's New in AnimalQuest</div>
...
async function submitFeedback() {
    ...
    try {
        await database.ref('animalquest/feedback').push({
            type: feedbackType,
            description: text,
...
function bindEventListeners() {
    ...
    document.getElementById('chatPolicyAgreeBtn')?.addEventListener('click', () => {
        try { localStorage.setItem('animalquest_chat_agreed', '1'); } catch(e) {}
...
async function generateUniqueGameCode() {
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
        ...
        const snapshot = await database.ref(`animalquest/games/${code}`).once('value');
...
async function createGame() {
    ...
    try {
        ...
        const roomRef = database.ref(`animalquest/games/${code}`);
        const roomData = {
...
function setupPresence() {
    ...
    if (presenceCleanup) { presenceCleanup(); presenceCleanup = null; }
    if (heartbeatInterval) { clearInterval(heartbeatInterval); heartbeatInterval = null; }
    const playerRef = database.ref(`animalquest/games/${currentGameCode}/players/${currentPlayer.playerKey}`);
...
function shareGameCode() { if (!currentGameCode) return; const joinUrl = getCanonicalJoinUrl(); const shareData = { title: 'Join my AnimalQuest Pack!', text: `Join my wildlife spotting pack! Use code: ${currentGameCode}`, url: joinUrl }; if (navigator.share) navigator.share(shareData).catch(() => copyToClipboard(`${shareData.text}\n${joinUrl}`, 'Join link copied! 📋')); else copyToClipboard(`${shareData.text}\n${joinUrl}`, 'Join link copied! 📋'); }
function inviteNewPlayer() { if (!currentGameCode) return; const message = `🐺 Join my AnimalQuest Pack!\n\nPack Code: ${currentGameCode}\nJoin here: ${getCanonicalJoinUrl()}\n\nLet's hunt wildlife together!`; if (navigator.share) navigator.share({ title: 'Join my AnimalQuest Pack!', text: message }).catch(() => copyToClipboard(message, 'Invitation copied to clipboard! 📋')); else copyToClipboard(message, 'Invitation copied to clipboard! 📋'); }
...
function openChat() {
    try {
        if (!localStorage.getItem('animalquest_chat_agreed')) {
...
async function assignGamePrizes(gameCode, plateScope) {
    ...
    await database.ref(`animalquest/games/${gameCode}`).update({ luckyPlate, chests });
}
...
function showEndGameScreen() {
    ...
    if (subtitleEl) subtitleEl.textContent = `${gameData?.name || 'AnimalQuest'} Pack · Final Results`;
...
function renderWinnersCanvas(sorted, pithyDesc) {
    ...
    const packLabel = (gameData?.name || 'AnimalQuest Pack') + '  ·  ' +
    ...
    ctx.font = '12px system-ui, -apple-system, sans-serif';
    ctx.fillText('🐺  AnimalQuest', W / 2, H - 10);
...
async function shareWinnersCircle() {
    ...
    canvas.toBlob(async blob => {
        ...
        const file = new File([blob], 'animalquest-winners.png', { type: 'image/png' });

        // Try image share
        try {
            if (navigator.share && navigator.canShare?.({ files: [file] })) {
                await navigator.share({ title: `AnimalQuest — ${gameData.name || 'Pack'} Results`, text: shareText, files: [file] });
                return;
            }
        } catch (e) {
        ...
        try {
            if (navigator.share) {
                await navigator.share({ title: 'AnimalQuest Results', text: shareText });
                return;
            }
        } catch (e) {
            if (e.name === 'AbortError') return;
        ...
        a.download = 'animalquest-winners.png';
